...
Player::Player(std::unique_ptr<Collider> collider, SDL_Texture* texture, Vector2D pos, Vector2D vel, double mass, enet_uint32 peerID) 
    : rotationSpeed(0), rotation(0), Entity(std::move(collider), texture, pos, vel, mass), peerID(peerID) { 
} 
